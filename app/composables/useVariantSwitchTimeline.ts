...
export function useVariantSwitchTimeline() {
  ...
  const removeDuration = useState<number>(
    SWITCH_REMOVE_DURATION_KEY,
    () => initialDurations.removeDuration,
  );
  const pauseDuration = useState<number>(
    SWITCH_PAUSE_DURATION_KEY,
    () => initialDurations.pauseDuration,
  );
  const typeDuration = useState<number>(
    SWITCH_TYPE_DURATION_KEY,
    () => initialDurations.typeDuration,
  );
