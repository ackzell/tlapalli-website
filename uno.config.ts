...
export default defineConfig({
  ...
  shortcuts: [
    [
      /^btn-tpl-(\w+)$/,
      ([, color]) =>
        `cursor-pointer px-4 py-2 rounded-md border-solid border-1 border-${color}-dark-ui-widgetBorder bg-${color}-dark-ui-widgetBg text-${color}-dark-ui-foreground hover:bg-${color}-dark-ui-widgetHoverBg hover:text-${color}-dark-ui-widgetHoverForeground active:bg-${color}-dark-ui-foreground active:text-${color}-dark-ui-widgetBorder`,
    ],
    [
      /^btn-tpl-(\w+)-light$/,
